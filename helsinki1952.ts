...
function helsinkiObjektum(elemek: string[]): helsinki52Elem[] {
    var beolvasottAdatok: helsinki52Elem[] = [];
    for (let i: number = 0; i < elemek.length; i++) {
        let daraboltSor: string[] = elemek[i].split(" ");
        let objektum: helsinki52Elem = {
...
var helsinkiAdatok: helsinki52Elem[] = helsinkiObjektum(eredmenyek);
...
// 3. feladat
function pontszerzo(adatok: helsinki52Elem[]): void {
...
// 4. feladat
function ermekSzama(adatok: helsinki52Elem[]): void {
    var aranydb: number = 0;
    var ezustdb: number = 0;
    var bronzdb: number = 0;
    ...
    var osszesdb: number = aranydb + ezustdb + bronzdb;
    console.log("4. feladat:" + '\n' + "Arany: " + aranydb + '\n' + "Ezüst: " + ezustdb + '\n' + "Bronz: " + bronzdb + '\n' + "össesen: " + osszesdb);
}

ermekSzama(helsinkiAdatok);
// 5. feladat
function olimpiaiPontokOsszesen(adatok: helsinki52Elem[]): void {
    var pontok: number = 0;
    for (let i: number = 0; i < adatok.length; i++) {
        if (adatok[i].helyezes == 1) {
            pontok += 7;
...
// 6. feladat
function uszasTornaTobbErem(adatok: helsinki52Elem[]): void {
    var uszasdb: number = 0;
    var tornadb: number = 0;
...
// 7. feladat
function legtobbPontszerzoStatisztika(adatok: helsinki52Elem[]): void {
